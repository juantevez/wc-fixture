...
func (s *Server) Start(ctx context.Context) error {
	...
	go func() {
		log.Info("servidor HTTP venue-geo iniciado", "port", s.cfg.Port)
	...
	case <-ctx.Done():
		...
		if err := srv.Shutdown(shutdownCtx); err != nil {
			return fmt.Errorf("venue_geo/http: error en shutdown: %w", err)
		}
		log.Info("servidor HTTP venue-geo detenido correctamente")
		return nil
	case err := <-errCh:
		return fmt.Errorf("venue_geo/http: error fatal: %w", err)
	}
}
